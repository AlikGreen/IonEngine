...
{
    public void Draw(int selectedObj)
    {
        ...
        if(selectedObj >= 0 && selectedObj < Window.scene.gameObjects.Count)
        {
            byte[] buffer = new byte[100];
            // Copy the string to the buffer
            Encoding.ASCII.GetBytes(Window.scene.gameObjects[selectedObj].name, buffer);

            if(ImGui.InputText("Label", buffer, 100))
            {
                Window.scene.gameObjects[selectedObj].name = Encoding.ASCII.GetString(buffer);
            }

            List<Component> components =
            [
                Window.scene.gameObjects[selectedObj].transform,
                .. Window.scene.gameObjects[selectedObj].components,
            ];

            for (int i = 0; i < components.Count; i++)
            {
                ImGui.PushID(i + Window.scene.gameObjects[selectedObj].components.Count);
                ImGui.Spacing();
                ImGui.Separator();
                ImGui.Spacing();

                ImGui.Selectable(components[i].GetType().Name);
                ...
                if (ImGui.BeginPopup("options"))
                {
                    ...
                    {
                         Window.scene.gameObjects[selectedObj].components[i-1].OnDelete();
                        Window.scene.gameObjects[selectedObj].components.RemoveAt(i-1);
                        i--;
                    }
                    ImGui.EndPopup();
                }
                ...
                ImGui.PopID();
                ImGui.PushID(i);
                DisplayFields(components[i], dragSpeed);
                ImGui.PopID();
            }

            var componentSubclasses = Assembly.GetExecutingAssembly().GetTypes().Where(t => t.IsSubclassOf(typeof(Component)) && !t.IsAbstract);
            ...
            {
                foreach (var type in componentSubclasses)
                {
                    if (type != typeof(Transform))
                    {
                        if (ImGui.Button(type.Name))
                        {
                            ...
                            if (component != null)
                            {
                                Window.scene.gameObjects[selectedObj].AddComponent(component);
                            }
                        }
                    }
                }
