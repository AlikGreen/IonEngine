...
namespace Neon::Editor
{
    ...
    void EditorViewportWindow::startup()
    {
        ...
        const auto viewDesc = RHI::TextureViewDesc(image.texture);
        ...
        m_objectPickingPass = makeBox<ObjectPickingPass>(m_device);
    }
    ...
    void EditorViewportWindow::render()
    {
        ...
        if(camera.renderTarget != nullptr)
        {
            ...
            if(ImGui::IsItemClicked())
            {
                ...
                RHI::TextureReadDesc readDesc{};
                readDesc.width = 1;
                readDesc.height = 1;
                readDesc.depth = 1;
                readDesc.x = static_cast<uint32_t>(relative.x);
                readDesc.y = static_cast<uint32_t>(relative.y);
                readDesc.pixelLayout = RHI::PixelLayout::RInt;
                readDesc.pixelType = RHI::PixelType::UnsignedInt;
    ...
    void EditorViewportWindow::event(Event *event)
    {
        ...
        {
            if (const auto entity = std::get_if<ECS::Entity>(&inspectEvent->inspected))
            {
    ...
    void EditorViewportWindow::updatePickerTexture(const uint32_t width, const uint32_t height)
    {
        ...
        const RHI::TextureDesc texDesc = RHI::TextureDesc::Texture2D(width, height, RHI::PixelFormat::R8Uint);
        const Rc<RHI::Texture> pickerTex = m_device->createTexture(texDesc);

        const auto viewDesc = RHI::TextureViewDesc(pickerTex);
